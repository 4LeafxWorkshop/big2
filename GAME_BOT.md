...
## Scope
This document describes how the Big Two bots and the in-game recommendation system work in the current codebase (`src/main.js`).
...
- Legal moves are filtered by:
  - First-trick rule (`must contain ♦️3`)
  - Beat rule (must beat current `lastPlay` if there is one)
- 5-card generation keeps one representative per rank for:
...
Priority rules:
1. If leading (no `lastPlay`), run finish-plan shortcut (`forceFinishPlanPlay`) to try an unbeatable single that leaves a one-shot finish group.
...
- When forcing max against a 1-card opponent, if the strongest legal move is a single `2` and your hand size > 1, prefer the strongest non-`2` single instead.
...
### `leadScore(play)`
Components:
- Base by card count: 5-card `30`, triple `20`, pair `14`, single `4`.
- 5-card kind bonus: `FIVE_KIND_POWER * 4`.
- Penalties:
  - Preserve high ranks (Q/K/A/2) while hand is large.
  - Breaking duplicates into singles early.
  - Early single usage when hand is still large.
- Bonuses:
  - Low-rank singles.
  - Absolute lowest single in hand.
  - Closeout bonus for small hands using pairs/triples/fives.

### `respondCost(play)`
Components:
- Penalize breaking duplicates for single responses (when hand is large).
- Penalize spending high ranks (Q/K/A/2) while hand is still large.
- Penalize using the highest single while hand > 3.
- Additional cost for spending a 5-card response.
...
### Current Level
Recommendation is hard-tuned:
- `suggestPlay(..., game)` first tries the hard bot logic (`chooseAiPlay(..., 'hard')`).
- If the hard pick is not legal in the current filtered set, it falls back to the scored model.

### Recommendation Filters (Before Scoring)
- Generate all valid plays from current hand.
- If first trick: keep only plays containing `♦️3`.
...
- Remaining high singles (Q/K/A/2): `-7` each
- Remaining 2s: `-12` each
- Remaining top 2 (♠️2): `-10` each
...
- If holding `♠️2` and any five-card play exists, penalize non-five-card leads by `-14`.
- If max rank >= Q and hand size > 5: `-10`.
- Lowest single: `+2`.
- Using any `2`: `+12` if blitz is active, otherwise `-18`.
- If control check is active and leading with a single or pair of `2`s, add `+10`.
- If the play is a spade flush, add `+12`.

Threat mode (`oppMin <= 2`):
- If max rank >= Q: `+8`; else if max rank >= 10: `+4`.
- If a single uses the highest card: `+6`.

Bait logic:
- If leading, holding at least two `2`s including `♠️2`, and the play is a non-`2` pair, add `+10`.

First trick (must include `♦️3`):
- If `♦️3` is a true singleton and hand size > 10, add `+25`.
- If playing the `♦️3` reduces overall singletons, add `+15`.
- If leading with triple `3`s, add `+30`.
- If leading with a five-card that includes `A` or `2`, apply `-30`.
- If leading with a straight flush that includes `♦️3`, apply `-50`.
...
- If a straight includes `A` or `2` and hand size > 5, apply `-25`.
- If the play is a spade flush, add `+15` (additional to the earlier +12).
...
- If leading and `hasControlCheck(hand)` is false, avoid using `2`s if a same-kind alternative is within 10 points.
...
- Suggest pass in selected high-cost response scenarios.
...
- Hard: mostly deterministic (except upstream randomness in game flow/deal).

## Operational Notes
- Bots use full internal game state available in the runtime.
- Recommendation uses game-state context for hard-level suggestions.
- Repeated recommend clicks can return different outputs if the game state or selection changes.
