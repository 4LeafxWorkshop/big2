...
export function renderHomeProfileCardHtml(params){
  ...
  const nameFieldHtml=signedIn
    ?`<label class="field field-compact"><span>${t('name')}</span><div class="name-with-google name-with-google-signed-in"><input id="name-input" value="${esc(String(state.home.name??''))}" maxlength="18"/><div class="name-auth-row name-auth-row-inline"><div id="google-name-inline"></div></div></div></label>`
    :`<label class="field field-compact"><span>${t('loginToStart')}</span><div class="name-with-google name-with-google-login"><div class="name-auth-row name-auth-row-login"><div id="google-name-inline"></div></div></div></label>`;
  return`<div class="home-form-col home-form-left home-section"><h3 class="home-section-title"><span class="title-icon title-icon-player" aria-hidden="true"></span>${t('playerSettings')}</h3><div class="home-profile-card"><div class="home-profile-avatar"><img id="home-avatar-img" src="${safeHomeAvatarSrc}" data-fallback="${safeHomeAvatarFallbackSrc}" referrerpolicy="no-referrer" crossorigin="anonymous" onerror="this.onerror=null;this.src=this.dataset.fallback" alt="${esc(state.home.name||t('name'))}"/></div><div class="home-profile-fields">${nameFieldHtml}<label class="field field-compact"><div class="option-combo toggle-combo" id="gender-combo"><span class="gender-voice-icon" aria-hidden="true" title="${t('audioVoice')}"><svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M4 14v-4h4l5-5v14l-5-5H4z"/><path d="M15.5 8.5c1.8 1.8 1.8 5.2 0 7"/><path d="M18.2 5.8c3.3 3.3 3.3 8.9 0 12.2"/></svg></span><button class="combo-btn toggle-btn ${state.home.avatarChoice==='male'?'active':''}" data-value="male">${t('male')}</button><button class="combo-btn toggle-btn ${state.home.avatarChoice==='female'?'active':''}" data-value="female">${t('female')}</button></div></label></div></div>${aiFieldLeft}${cardBackLeft}</div>`;
}
