...
export function renderIntroPanel(params){
  ...
  const historyBlocks=String(intro.historyBody??'')
    ...
    .join('');
  const installPreview=`<div class="intro-home-preview"><img class="intro-home-icon" src="${withBase('icons/icon-192.png')}" alt="${esc(intro.guideHomeTitle)}"/><div class="intro-home-meta"><strong>${esc(intro.guideHomeTitle)}</strong><span>${esc(appTitle)}</span></div></div>`;
