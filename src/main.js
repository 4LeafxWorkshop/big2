...
const FIRESTORE_ROOM_PLAYERS_COLLECTION='big2RoomPlayers';
...
async function readRoomPlayerPointer(playerId){
  if(!firebaseDb||!playerId)return null;
  try{
    const snap=await firebaseDb.collection(FIRESTORE_ROOM_PLAYERS_COLLECTION).doc(String(playerId)).get();
    return snap.exists?snap:null;
  }catch{
    return null;
  }
}
async function writeRoomPlayerPointer(playerId,roomId,firebaseInstanceId=''){
  if(!firebaseDb||!playerId)return false;
  const safeRoomId=String(roomId||'').trim();
  const safeInstanceId=String(firebaseInstanceId||state.room?.firebaseInstanceId||primaryFirebaseInstanceId()).trim()||primaryFirebaseInstanceId();
  await firebaseDb.collection(FIRESTORE_ROOM_PLAYERS_COLLECTION).doc(String(playerId)).set({
...
async function deleteRoomPlayerPointer(playerId){
  if(!firebaseDb||!playerId)return;
  try{
    await firebaseDb.collection(FIRESTORE_ROOM_PLAYERS_COLLECTION).doc(String(playerId)).delete();
  }catch{}
}
...
const googleIdentityController=createGoogleIdentityController({
  ...
  appleSignIn:async()=>{
    ...
    try{
      ...
      await handleFirebaseOAuthUser({
        provider:'apple',
        user,
        email:String(user?.email??profile?.email??''),
        name:String(user?.displayName??profile?.name??profile?.fullName??state.home.name??''),
        picture:String(user?.photoURL??profile?.picture??''),
        sub:String(user?.uid??profile?.sub??'')
      });
      return true;
    }catch(err){
...
async function findRoomByPlayerId(playerId){
  ...
  if(safePlayerId.startsWith('uid:')){
    const authUid=safePlayerId.slice(4).trim();
    if(!authUid)return null;
    try{
      const snap=await firebaseDb.collection(FIRESTORE_USERS_COLLECTION).doc(authUid).get();
      const data=snap.data()??{};
  ...
  const pointer=await readRoomPlayerPointer(safePlayerId);
  if(!pointer)return null;
  const pointerData=pointer.data()??{};
  const roomId=String(pointerData.roomId??'').trim();
  if(!roomId){
    await deleteRoomPlayerPointer(safePlayerId);
    return null;
  }
  const resolved=await resolveRoomDocByDirectory(roomId,'');
  if(!resolved){
...
async function dropSelfFromRoom(roomDoc,playerId){
  ...
  if(String(playerId).startsWith('guest:'))await deleteRoomPlayerPointer(playerId);
}
...
async function updateActiveRoomPointer(roomId,instanceId=''){
  ...
  if(!uid){
    ...
    if(roomId)await writeRoomPlayerPointer(playerId,roomId,instanceId);
    else await deleteRoomPlayerPointer(playerId);
    return;
  }
...
function authProviderBadgeHtml(provider){
  if(String(provider??'').trim().toLowerCase()==='apple'){
    return `<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path fill="currentColor" d="M15.6 2.7c-.9.1-2 .7-2.6 1.4-.6.7-1.1 1.8-1 2.9 1 .1 2-.4 2.7-1.1.7-.7 1.1-1.7.9-3.2ZM19.8 16.2c-.6 1.5-1.3 2.9-2.4 4.1-1 .9-2 1.4-3.3 1.4s-1.6-.2-2.5-.5c-.6-.2-1.1-.4-1.6-.4s-1 .2-1.6.4c-.9.3-1.7.5-2.5.5-1.3 0-2.3-.5-3.3-1.4C1.8 18.7 0 15.5 0 12.1c0-3.3 1.8-5.3 3.5-6.3.9-.5 1.9-.8 2.9-.8.9 0 1.6.2 2.2.4.5.2 1 .4 1.4.4s.8-.2 1.4-.4c.8-.3 1.7-.5 2.7-.5 1.1 0 2.3.3 3.3.9-.9.7-1.6 1.8-1.6 3.1 0 1.5.8 2.7 1.8 3.5.7.6 1.3 1 2.2 1-.1 1-.4 1.9-.8 2.8Z"/></svg>`;
  }
  return `<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path fill="#4285F4" d="M23.49 12.27c0-.79-.07-1.54-.2-2.27H12v4.31h6.44a5.5 5.5 0 0 1-2.39 3.61v3h3.86c2.26-2.08 3.58-5.15 3.58-8.65Z"/><path fill="#34A853" d="M12 24c3.24 0 5.95-1.07 7.93-2.91l-3.86-3A7.17 7.17 0 0 1 12 19.3c-3.12 0-5.77-2.11-6.72-4.96H1.3v3.11A12 12 0 0 0 12 24Z"/><path fill="#FBBC05" d="M5.28 14.34a7.2 7.2 0 0 1 0-4.68V6.55H1.3v10.9l3.98-3.11Z"/><path fill="#EA4335" d="M12 4.77c1.76 0 3.34.61 4.58 1.8l3.43-3.43C17.94 1.23 15.24 0 12 0A12 12 0 0 0 1.3 6.55l3.98 3.11C6.23 6.88 8.88 4.77 12 4.77Z"/></svg>`;
}
...
function renderHome(){
  ...
  const soloReadySubtitle=soloStartPending
    ?t('startingGame')
    :(profileRestorePending?t('restoringScore'):t('startReadySubtitle'));
  const soloBtnCore=`<button id="solo-start" class="primary royal-start-btn" ${signedIn&&!profileRestorePending&&!soloStartPending?'':'disabled'}><span class="home-btn-main">${t('solo')}</span><span class="home-btn-subtitle">${esc(soloReadySubtitle)}</span></button>`;
