...
function renderGame(){
  ...
  const seatHtml=arr.filter((p)=>p.viewIndex!==0).map((p)=>{
    ...
    const fanAnchorStyle=isSideSeat
      ?'position:absolute !important;top:calc(22% + 72px) !important;left:50% !important;right:auto !important;transform:translateX(-50%) !important;'
      :(p.cls==='north'
    ...
    const openAnchorStyle=isSideSeat
      ?'position:absolute !important;top:calc(22% + 164px) !important;left:50% !important;right:auto !important;width:max-content !important;transform:translateX(-50%) !important;'
      :'';
